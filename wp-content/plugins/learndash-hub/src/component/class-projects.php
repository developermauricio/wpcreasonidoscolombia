...
class Projects extends Base {
		...
		foreach ( $api_data as $slug => &$item ) {
			...
			if ( 'learndash-hub' === $slug ) {
				...
				if ( file_exists( $mu_path ) ) {
					$plugin_data = get_plugin_data( WPMU_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'learndash-hub-mu.php' );
				} else {
					// this was installed as a normal plugin.
					$plugin_data = get_plugin_data( WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . $plugin_slug );
				}
			} else {
				$plugin_data = get_plugin_data( WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . $plugin_slug );
			}
	...
	public function get_plugin_slug( string $slug, string $plugin_name = '' ) {
		...
		foreach ( $plugins as $folder_slug => $plugin ) {
			if ( strtolower( $plugin['Author'] ) === 'learndash' && strtolower( $plugin_name ) === strtolower( $plugin['Name'] ) ) {
				return $folder_slug;
			}
		}
